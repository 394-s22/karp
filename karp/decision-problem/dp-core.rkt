...
;
; Core kv type object definitions
; Note: place the definitions here because the rewriters only can refer to them
; after their defined
;
...
; type object representin Symbol
...
;
; TODO: move this section to a approperiate place
;
...
(define (set-∈ a-element a-set)
  (hash-ref (dp-set-S (as-set a-set)) a-element #f))
(define (set-∉ a-element a-set)
...
;
; Problem definition and solver derivation 
;
...
; Note: unbounded natural is no longer supported as solvable
; Begin: Not-In-Use
; -----------------------------------------------------
;

(define dp-null-natural -1)
(define (natural/c x)
  (λ (x)
    (or (natural? x)
        (equal? x dp-null-natural))))
...
(define (dp-natural-from-sol sym-n a-sol)
  (if (r:unsat? a-sol) dp-null-natural
      ; use 0 as default
...
(provide [rename-out (random random-int)])
...
; XXX same bottom element as natural
...
(define (gen-random-set el-gen #:size [size #f])
  (let ([size (if size size (random 6 13))])
    (let rec ([cur '()]
              [k size]
              [cnt 0])
                 ...
                 [set-ctc
                  (if size-as-num
                      #`(make-flat-contract
                         ...
                         (λ (blame)
                           (λ (a-set neg-party)
                             (if (equal? (set-size a-set) #,size-as-num)
                      ...
                      #`(λ (a-inst)
                          (make-flat-contract
                           ...
                           (λ (blame)
                             ...
                             (λ (a-set neg-party)
                               (if (equal? (set-size a-set) #,(instance-field-ref #'a-inst maybe-size))
                              ...
                              (random-subset
                               ...
                               #,(if maybe-size
                                     (if size-as-num
                                         size-as-num
                                         (instance-field-ref #'a-inst maybe-size))
                                     #'(random (quotient (set-size superset) 2)
                                               (set-size superset)))
                               #:exact-n? #f))))
